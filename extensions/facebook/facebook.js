...
async function* fetchAttachments(story) {
  if (isUnifiedStory(story)) {
    for (const attachment of story.attachments) {
      if (attachment.media) yield attachment.media;
    }
    return;
  }
...
/**
 * Fetch Facebook story data for a specific story ID.
 * @param {string} storyId
 * @returns {Promise<any[]>}
 */
async function fetchFacebookStoryData(storyId) {
  const match = window.location.href.match(/facebook\.com\/stories\/(\d+)/);
  const bucketID = match ? match[1] : storyId;
  ...
  const matchStory = extracted.find((s) => getStoryId(s) === storyId);
  if (matchStory) {
    console.log(
      `[fpdl] Found matching Facebook story ${storyId} in bucket ${bucketID}.`,
    );
    return [matchStory];
  }
